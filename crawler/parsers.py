...
# 제목 표시 정제용: 접수방식(이메일접수 등) 및 마감일자/기간 괄호 표기 제거
APPLY_METHOD_RE = re.compile(
    r"\s*[\(\[][^\)\]]*(?:이메일|전자우편|방문|우편|온라인|인터넷|원서|서류)[^\)\]]*접수[^\)\]]*[\)\]]",
    re.I
)
DATE_RANGE_PAREN_RE = re.compile(
    r"\s*[\(\[]\s*~?\s*(?:\d{2,4}\s*[.\-/년]\s*)?\d{1,2}\s*[.\-/월]\s*\d{1,2}\s*일?\s*(?:\([월화수목금토일]\))?\s*(?:까지)?\s*[\)\]]"
    r"|\s*[\(\[]\s*(?:\d{2,4}\s*[.\-/년]\s*)?\d{1,2}\s*[.\-/월]\s*\d{1,2}\s*일?\s*(?:\([월화수목금토일]\))?\s*~\s*(?:\d{2,4}\s*[.\-/년]\s*)?\d{1,2}\s*[.\-/월]\s*\d{1,2}\s*일?\s*(?:\([월화수목금토일]\))?\s*(?:까지)?\s*[\)\]]"
    r"|\s*[\(\[]\s*~[^\)\]]*\.\.\.\s*$"
    r"|\s*~\s*(?:\d{2,4}\s*[.\-/년]\s*)?\d{1,2}\s*[.\-/월]\s*\d{1,2}\s*일?\s*(?:\([월화수목금토일]\))?\s*(?:까지)?\s*$"
)
PREFIX_TAG_RE = re.compile(
    r"^\s*(?:(?:\[|\(|\<|【)\s*(?:공지|공지사항|채용|채용공고|모집|공개모집|긴급|긴급채용|채용정보|인재채용|채용\/모집|채용\/공고)\s*(?:\]|\)|\>|】)\s*|(?:공지|공지사항|채용공고)\s*[:-]?\s+)+",
    re.I
)


def clean_display_title(title):
    """제목에서 (이메일접수), 마감일자(~9.15, 9.1~9.11) 등의 부가 표기를 정제하고,
    앞머리의 불필요한 말머리 태그([채용], [공지], 공지 등)를 일괄 제거.
    직무 조건(사서, 기간제, 8급 등), 소속 기관명 및 임용일은 온전하게 보존한다."""
    if not title:
        return ""
    t = APPLY_METHOD_RE.sub("", title)
    t = DATE_RANGE_PAREN_RE.sub("", t)
    t = PREFIX_TAG_RE.sub("", t)
